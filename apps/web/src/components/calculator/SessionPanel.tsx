...
export default function SessionPanel({ recipes, ingredients, molds }: Props) {
  ...
  // 레시피별 선택된 몰드 ID
  const [moldSelections, setMoldSelections] = useState<
  ...
  function getAvailableCells(excludeRecipeId: string): Record<string, number> {
    ...
    for (const [recipeId, optionKey] of Object.entries(moldSelections)) {
      ...
      const selectedMolds = moldIds
        .map((id) => molds.find((m) => m.id === id))
      ...
      if (selectedMolds.length === 1) {
        ...
        available[mold.id] = Math.max(
          0,
          (available[mold.id] ?? mold.cellCount) - cellsUsed,
        );
      } else {
        const [first, second] = selectedMolds;
        available[first.id] = Math.max(
          0,
          (available[first.id] ?? first.cellCount) - first.cellCount,
        );
        ...
        if (remainder > 0) {
          ...
          available[second.id] = Math.max(
            0,
            (available[second.id] ?? second.cellCount) - secondCellsUsed,
          );
        }
      }
    }
        ...
        {items.map((item) => {
          ...
          return (
            <li key={item.recipeId}>
              <Card
                borderColor={
                  hasShortage ? 'border-red-200 dark:border-red-900' : undefined
                }
              >
                {/* 상단: 레시피명 + 배율 + 삭제 */}
                  ...
                  <div className="border-t border-zinc-100 px-3 pb-3 pt-2.5 dark:border-zinc-800">
                    <ul className="flex flex-col gap-2">
                      {results.map((r) => {
                        ...
                        const originalId =
                          r.originalIngredientId ?? r.ingredientId;
                        ...
                        // 범위 재료 여부
                        const isRange =
                          recipeIng?.amountMin != null &&
                          recipeIng?.amountMax != null;

                        // 대체재료 이름
                        const substituteName = substituteDef
                          ? ingredientMap.get(
                              substituteDef.substituteIngredientId,
                            )?.name
                          : null;

                        // 오버라이드 여부 (리셋 버튼 표시 기준)
                        const isOverridden =
                          item.amountOverrides?.[originalId] != null;
                          ...
                          >
                                ...
                                {isRange && recipeIng ? (
                                  <div className="relative flex items-center">
                                    <input
                                      type="number"
                                      value={
                                        editingAmounts[amountKey] ?? r.required
                                      }
                            ...
                            {isRange && recipeIng && (
                              <p className="text-[11px] text-zinc-300 dark:text-zinc-600">
                                {recipeIng.amountMin}~{recipeIng.amountMax}
                                {r.unit}
                              </p>
                            )}
                            ...
                            {substituteDef && substituteName && (
                              ...
                              >
                                {isSubstituteActive
                                  ? `↩ 원래: ${ingredientMap.get(originalId)?.name ?? r.name}`
                                  : `↔ 대체: ${substituteName}`}
                              </button>
                            )}
                          </li>
                    ...
                    {moldOptions.length > 0 && (
                      <div className="mt-2 flex items-start gap-2 border-t border-zinc-100 pt-2 dark:border-zinc-800">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {moldOptions.map(
                            ({ molds: optionMolds, cellsMap, isAvailable }) => {
                              const optionKey = optionMolds
                                .map((m) => m.id)
                                .join('+');
